...
class SignatureViewState extends State<SignatureView> {
            ...
            title: Column(
              ...
              children: [
                Text('${locale.order} - #${orderDetails!.cartId}',
                    // 'Order',
                    style: Theme.of(context).textTheme.headlineMedium!.copyWith(fontWeight: FontWeight.w500, color: kMainTextColor, fontSize: 12)),
                const SizedBox(
                  height: 5,
                ),
                Text('${locale.order} ${locale.invoice3h} - $apCurency ${orderDetails!.remainingPrice!.toStringAsFixed(2)}',
                    // 'Order',
                    style: Theme.of(context).textTheme.headlineMedium!.copyWith(fontWeight: FontWeight.w300, color: kMainTextColor, fontSize: 13)),
              ],
            ),
            actions: <Widget>[
              Padding(
  ...
  void uploadSignature(context, AppLocalizations locale) async {
    ...
    if(data != null) {
      File image = await File('${directory.path}/image.png').writeAsBytes(data);
      ...
      var formData = FormData.fromMap(
        {
          ...
          'user_signature': await MultipartFile.fromFile(image.path),
        },
      );
      ...
      debugPrint('Unable to get user signature image. Function uploadSignature line 216');
    }
  }
}
